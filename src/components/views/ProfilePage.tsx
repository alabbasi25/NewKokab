...
import { KokabToggle } from '../ui/KokabToggle';
import { 
  BellRing, 
  Wallet, 
  MessageCircle, 
    ...
    >
      ...
      <div className="space-y-4">
        ...
        <div className="glass-card divide-y divide-[var(--color-border)]">
          ...
          <NotificationSettingsSection profile={profile} updateProfile={updateProfile} />
...
const SettingsItem: React.FC<{ icon: React.ReactNode; label: string; description: string; onClick?: () => void }> = ({ icon, label, description, onClick }) => (
  <button 
    onClick={onClick}
...
const NotificationSettingsSection: React.FC<{ profile: any; updateProfile: (p: any) => void }> = ({ profile, updateProfile }) => {
  const [isOpen, setIsOpen] = useState(false);

  const toggleSetting = (key: keyof typeof profile.notificationSettings) => {
    updateProfile({
      ...profile,
  ...
  return (
    <div>
      <SettingsItem 
        icon={<Bell size={20} />} 
        label="الإشعارات" 
        description="تخصيص تنبيهات النظام والشركاء" 
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="overflow-hidden bg-[var(--color-bg-deep)]/40 px-4 py-2 space-y-2 border-b border-[var(--color-border)]"
          >
            <KokabToggle 
              label="تنبيهات المهام" 
              description="عند تكليف أو إكمال مهمة متبادلة"
              enabled={profile.notificationSettings.tasks}
              onChange={() => toggleSetting('tasks')}
              icon={<BellRing size={18} />}
            />
            <KokabToggle 
              label="التنبيهات المالية" 
              description="عند تسجيل معاملات أو طلبات توافق"
              enabled={profile.notificationSettings.financial}
              onChange={() => toggleSetting('financial')}
              icon={<Wallet size={18} />}
            />
            <KokabToggle 
              label="نشاط الشريك" 
              description="تنبيهات الدردشة، التواجد، والنبض الهابتك"
              enabled={profile.notificationSettings.social}
              onChange={() => toggleSetting('social')}
              icon={<HeartHandshake size={18} />}
            />
            <KokabToggle 
              label="الأذكار والأوراد" 
              description="تذكيرات بأوقات الأذكار المجدولة"
              enabled={profile.notificationSettings.athkar}
              onChange={() => toggleSetting('athkar')}
              icon={<Clock size={18} />}
            />
          </motion.div>
        )}
