    ...
    <div className="fixed inset-0 bg-[#0f172a] flex flex-col z-[100] text-white overflow-hidden font-sans rtl" style={{ direction: 'rtl' }}>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {isPartnerTyping && (
                      <div className="flex gap-0.5 ml-1">
                         ...
                         <motion.div animate={{ y: [0, -3, 0] }} transition={{ repeat: Infinity, duration: 0.6, delay: 0.4 }} className="w-1 h-1 bg-emerald-400 rounded-full" />
                      </div>
                    )}
                  </div>
                </div>
      ...
      >
        ...
        {isPartnerTyping && (
          ...
          >
            <div className="w-8 h-8 rounded-full overflow-hidden shrink-0 border border-emerald-500/20">
               <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${partnerName}`} alt="Typing" />
            </div>
            <div className="bg-[#1e293b] px-4 py-2 rounded-2xl rounded-tr-none flex items-center gap-1.5 border border-white/5 shadow-xl">
               <motion.div animate={{ y: [0, -4, 0] }} transition={{ repeat: Infinity, duration: 0.8, delay: 0 }} className="w-1.5 h-1.5 bg-white/40 rounded-full" />
               <motion.div animate={{ y: [0, -4, 0] }} transition={{ repeat: Infinity, duration: 0.8, delay: 0.2 }} className="w-1.5 h-1.5 bg-white/40 rounded-full" />
               <motion.div animate={{ y: [0, -4, 0] }} transition={{ repeat: Infinity, duration: 0.8, delay: 0.4 }} className="w-1.5 h-1.5 bg-white/40 rounded-full" />
            </div>
          </motion.div>
        )}
      </div>
...
const MessageBubble: React.FC<MessageBubbleProps> = ({ 
  ...
  onReplyClick, playingAudioId, onToggleAudio, onForward, highlightMatch, searchTerm
}) => {
        ...
        >
          ...
          {msg.type === 'audio' ? (
            ...
            />
             ...
             <div className="text-[14px] leading-relaxed break-words text-right whitespace-pre-wrap">
...
const CustomAudioPlayer: React.FC<{ 
...
}> = ({ url, isOwn, id, playingAudioId, onToggle }) => {
  ...
  return (
    <div className="flex items-center gap-3 min-w-[220px] py-1" onClick={e => e.stopPropagation()}>
      <audio 
        ref={audioRef}
        src={url}
      ...
      <motion.button 
        ...
        className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition-all ${
          isOwn ? 'bg-white/20 hover:bg-white/30 text-white' : 'bg-emerald-500/20 text-emerald-500 hover:bg-emerald-500/30'
        }`}
      >
      ...
      <div className="flex-1 flex flex-col gap-1">
        <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden relative">
          <motion.div 
            ...
            className={`absolute top-0 left-0 h-full ${isOwn ? 'bg-white' : 'bg-emerald-500'}`}
          />
        </div>
        <div className="flex justify-between items-center text-[8px] font-black opacity-60 font-mono tracking-tighter">
          ...
          <div className="flex gap-0.5">
             {[...Array(6)].map((_, i) => (
                <motion.div 
                   ...
                   className={`w-0.5 rounded-full ${isOwn ? 'bg-white/40' : 'bg-emerald-500/40'}`}
                />
             ))}
          </div>
